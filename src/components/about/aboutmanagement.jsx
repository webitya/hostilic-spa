...
const services = [
  ...
  {
    title:
      'Holistic Massage, Ayurveda, Beauty Treatments, Salon Services & Wedding Makeovers',
    description:
...
export default function AboutManagement() {
  return (
    <section
      className="relative py-12 px-4 md:px-8 overflow-hidden"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mx-auto">
          {services.map(({ title, description, icon: Icon }, idx) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className={idx === services.length - 1 ? 'md:col-span-2' : ''}
              style={{
                background:
